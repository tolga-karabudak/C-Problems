...
void readFile(depart_t *dep, FILE *inp)
{
	...
	for (i = 0; i < DEPT; i++)
	{
		...
		fscanf(inp, "%[^\n]s", (dep + i)->d_name);
		for (j = 0; j < EMP; j++)
			fscanf(inp, "%d %s %d %d", &(dep + i)->worker[j].id, 
			(dep + i)->worker[j].name, &(dep + i)->worker[j].age, 
			&(dep + i)->worker[j].worked_hr);
	}
}
	...
	for (j = 0; j < DEPT; j++)
	{
		for (i = 0; i < EMP; i++)
		{
			if (dep[j].worker[i].worked_hr <= HRS)
				dep[j].worker[i].salary = 
				dep[j].worker[i].worked_hr * PER;
			else
				dep[j].worker[i].salary = 
				HRS * PER + (dep[j].worker[i].worked_hr - HRS) * OVER;
		}
	}
}
